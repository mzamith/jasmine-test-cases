...
Television.prototype.play = function (movie) {
    ...
    //if no movie is paused and no movie is paused
    //should throw an error
    } else {
        throw new Error ("Please select a movie to watch");
    }
};
...
Television.prototype.pause = function () {
    ...
    if (!this.isPlaying) throw new Error ("Movie is already paused");
    this.isPlaying = false;
};
...
Television.prototype.makeFavorite = function () {
    if (this.currentlyPlayingMovie){
        this.currentlyPlayingMovie.setFavorite();
    }else{
        throw new Error("Not watching any movie")
    };
};